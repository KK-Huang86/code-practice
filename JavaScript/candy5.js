...
function squareDigits(num) {
  ...
  const toBeString = num.toString(); //將傳入的引數轉為字串
  const toBeArray = Array.from(toBeString); //再將字串轉為陣列
  const square = (x) => {
    //定義一個函數來計算平方
    return x * x;
  };
  const squareNumber = toBeArray.map(square); //使用map語法，將裡面的數字都丟出去做平方，形成新的陣列
